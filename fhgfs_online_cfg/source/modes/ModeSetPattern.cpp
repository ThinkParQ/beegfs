...
#define MODESETPATTERN_ARG_STORAGEPOOL       "--storagepool"
...
int ModeSetPattern::execute()
{
   ...
   if(iter != cfg->end() )
   {
      ...
      bool poolExists = Program::getApp()->getStoragePoolStore()->poolExists(storagePoolId);
...
void ModeSetPattern::printHelp()
{
   ...
   std::cout << "  --storagepool=<poolID>       Use poolID as storage pool for new files in this" << std::endl;
   ...
   std::cout << " enabled `sysAllowUserSetPattern`." << std::endl;
   std::cout << std::endl;
