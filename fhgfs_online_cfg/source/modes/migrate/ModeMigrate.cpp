...
int ModeMigrate::getFromTargets(UInt16List& targetIDs, NumNodeIDList& nodeIDs)
{
   ...
   else if (cfgStoragePoolId != 0)
   {
      ...
      const UInt16Set targets = storagePoolStore->getPool(cfgStoragePoolId)->getTargets();
...
void ModeMigrate::printHelpMigrate()
{
   ...
   std::cout << "  --nodeid=<nodeID>          Migrate files away from all targets attached to this" << std::endl;
   std::cout << "                             nodeID." << std::endl;
   std::cout << "  --storagepoolid=<poolId>   Migrate files away from all targets belonging to the" << std::endl;
   std::cout << "                             storage pool with this ID." << std::endl;
   std::cout << "  --destinationpool=<poolId> Migrate files to targets from the storage pool with " << std::endl;
   std::cout << "                             this ID (Default: " <<
