   ...
   const struct mg_request_info *request_info, const char* postBuf, size_t postBufLen)
{
   ...
   Program::getApp()->getLogger()->log(Log_DEBUG, __func__, logMessage);
}
   ...
   TiXmlDocument doc)
{
   ...
   Program::getApp()->getLogger()->log(Log_DEBUG, __func__, "Requested URI: " +
   ...
   Program::getApp()->getLogger()->log(Log_DEBUG, __func__, "XML content: " + outStr);
#endif
}
