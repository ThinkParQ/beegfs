...
{
   Program::getApp()->getLogger()->log(Log_ERR, __func__, "Requested URI: " +
   ...
   const struct mg_request_info *request_info, const char* postBuf, size_t postBufLen)
{
   ...
   if (nonceID.empty())
   {
      ...
      Program::getApp()->getLogger()->log(Log_ERR, __func__, "Authentication failed. Missing nonce "
         "in request.");
      return;
   }
   ...
   const struct mg_request_info *request_info, const char* postBuf, size_t postBufLen)
{
   ...
   if(checkRes)
   {
      Program::getApp()->getLogger()->log(Log_ERR, __func__, "Failed to check distri "
         "architecture.");
   }
   ...
   const struct mg_request_info *request_info, const char* postBuf, size_t postBufLen)
{
   ...
   try
   {
      ...
      if(!nodes.empty() )
         Program::getApp()->getLogger()->log(Log_ERR, __func__, "SSH connection to some nodes "
            "failed. Nodes:" + nodes);
