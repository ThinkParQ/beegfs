...
class ChunkStore
{
   public:
      ...
      FhgfsOpsErr openChunkFile(int targetFD, Path* chunkDirPath, std::string& chunkFilePathStr,
         bool hasOrigFeature, int openFlags, int* outFD, SessionQuotaInfo* quotaInfo);

      bool chmodV2ChunkDirPath(int targetFD, Path* chunkDirPath, std::string& entryID);
   ...
   private:
      ...
      bool mkdirV2ChunkDirPath(int targetFD, Path* chunkDirPath);
