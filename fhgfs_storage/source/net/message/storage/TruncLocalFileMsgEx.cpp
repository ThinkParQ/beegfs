...
FhgfsOpsErr TruncLocalFileMsgEx::truncFile(int targetFD, Path* chunkDirPath,
   ...
   bool* outChunkLocked)
{
   ...
   *outChunkLocked = storageTargets->isBuddyResyncInProgress(actualTargetID);
      ...
      (rrTarget.outTargetReachabilityState == TargetReachabilityState_OFFLINE) ) )
   {
      ...
      // at this point:
      // actualTargetID = our own target and (as message assumes) primary
      // getTargetID() = mirrorGroupID
