   ...
   unsigned basePathLen)
{
   ...
   if(!dir)
   {
      LOG(WARNING, "Could not open directory.", path, targetID, sysErr());
   ...
   while(true && !getSelfTerminate())
   {
      ...
      if(readRes != 0)
      {
         LOG(WARNING, "readdir failed.", path, targetID,
      ...
      if(statRes)
      {
         LOG(WARNING, "Could not stat directory.", path, targetID, sysErr());
         result = false;
         break;
      }
