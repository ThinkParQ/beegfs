...
#define LOG_CTX_TOP_L(Topic, Level, Context, Line, Message, ...) \
   do { \
      Logger* const _log_logger = PThread::getCurrentThreadApp()->getLogger(); \
      if (_log_logger->getLogLevel(Topic) < Level) \
         break; \
...
#define LOG_CTX_L(Level, Context, Line, Message, ...) \
   LOG_CTX_TOP_L(LogTopic_GENERAL, Level, Context, Line, Message, ## __VA_ARGS__)

#define LOG(Level, Message, ...) \
   LOG_TOP(GENERAL, Level, Message, ## __VA_ARGS__)

#define LOG_CTX(Level, Context, Message, ...) \
   LOG_CTX_TOP(GENERAL, Level, Context, Message, ## __VA_ARGS__)

#define LOG_TOP(Topic, Level, Message, ...) \
   LOG_CTX_TOP_L(LogTopic_##Topic, Log_##Level, __FILE__, __LINE__, Message, ## __VA_ARGS__)

#define LOG_CTX_TOP(Topic, Level, Context, Message, ...) \
   ...
   #define LOG_DEBUG(Context, Level, Message) LOG_CTX_L(LogLevel(Level), &Context[0], -1, Message)
   #define LOG_DBG LOG
