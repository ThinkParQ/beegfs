...
bool MessagingTk::requestResponse(RequestResponseArgs* rrArgs)
{
   ...
   while (true)
   {
      const FhgfsOpsErr commRes = requestResponseComm(rrArgs);

      // On success, return true - not done inside switch() because we want to take advantage of the
      // likely() optimization hint.
      if (likely(commRes == FhgfsOpsErr_SUCCESS) )
         return true;

      switch (commRes)
      {
         case FhgfsOpsErr_AGAIN:
            { // Abort on selfTerminateOrder.
               const bool shallTerminate =
                  PThread::getCurrentThreadApp()->waitForSelfTerminateOrder(
         ...
         case FhgfsOpsErr_COMMUNICATION:
            // Log if LogFlag RETRY was set and we're on the first try.
            if (!retry && !(rrArgs->logFlags & REQUESTRESPONSEARGS_LOGFLAG_RETRY) )
               LogContext(logContext).log(Log_WARNING,
                  "Retrying communication with node: " + rrArgs->node->getNodeIDWithTypeStr() + " "
                  "(Message type: " + rrArgs->requestMsg->getMsgTypeStr() + ")");

            if (retry) // Abort after second try.
               return false;

            // One retry in case the connection was already broken when we got it (e.g. peer daemon
            // restart).
            retry = true;
            continue;

         default:
            // Particularly FhgfsOpsErr_WOULDBLOCK...
            return false; // Don't retry if it wasn't a comm error.
      }
   }
}
   ...
   RequestResponseArgs* rrArgs)
{
   ...
   while (true)
   {
      ...
      switch(commRes)
      {
         ...
         case FhgfsOpsErr_COMMUNICATION:
            if (!retry) // Log if we're on the first try.
               LogContext(logContext).log(Log_WARNING,
         ...
         default:
            return commRes; // Don't retry if it wasn't a comm error.
      }
   }
}
   ...
   RequestResponseArgs* rrArgs)
{
   ...
   while (true)
   {
      ...
      if (!loadedNode)
      {
         LOG(WARNING, "Unable to resolve storage server", targetID);
         return FhgfsOpsErr_UNKNOWNNODE;
      }
      ...
      switch (commRes)
      {
         ...
         case FhgfsOpsErr_COMMUNICATION:
            if (!retry) // Log if we're on first try
               LogContext(logContext).log(Log_WARNING,
