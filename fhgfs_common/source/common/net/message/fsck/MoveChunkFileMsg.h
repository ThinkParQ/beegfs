...
class MoveChunkFileMsg: public NetMessageSerdes<MoveChunkFileMsg>
{
   ...
   public:
    ...
    virtual TestingEqualsRes testingEquals(NetMessage* msg)
    {
       ...
       return chunkName == msgIn->chunkName &&
            oldPath == msgIn->oldPath &&
            newPath == msgIn->newPath &&
            targetID == msgIn->targetID &&
            isMirrored == msgIn->isMirrored &&
            overwriteExisting == msgIn->overwriteExisting
