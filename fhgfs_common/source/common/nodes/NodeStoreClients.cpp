 ...
 */
NodeHandle NodeStoreClients::referenceNode(NumNodeID numNodeID)
{
 ...
 * @return can be NULL
 */
NodeHandle NodeStoreClients::referenceFirstNode()
{
   std::lock_guard<Mutex> lock(mutex);
 ...
 */
NodeHandle NodeStoreClients::referenceNextNode(const NodeHandle& oldNode)
{
   std::lock_guard<Mutex> lock(mutex);
...
std::vector<NodeHandle> NodeStoreClients::referenceAllNodes()
{
   std::lock_guard<Mutex> lock(mutex);
...
bool NodeStoreClients::isNodeActive(NumNodeID nodeNumID)
{
   std::lock_guard<Mutex> lock(mutex);
 ...
 */
NumNodeID NodeStoreClients::retrieveNumIDFromStringID(std::string nodeID)
{
   for (auto iter = activeNodes.begin(); iter != activeNodes.end(); iter++)
   {
 ...
 * otherwise.
 */
bool NodeStoreClients::checkNodeNumIDCollision(NumNodeID numID)
{
