...
struct StripePatternHeader
{
   ...
   StripePatternHeader(StripePatternType type, uint32_t chunkSize)
...
class StripePattern
{
   ...
   protected:
      ...
      StripePattern(StripePatternType type, unsigned chunkSize)
         : header(type, chunkSize ? chunkSize : STRIPEPATTERN_DEFAULT_CHUNKSIZE)
      {
      }

      // (de)serialization
      static StripePattern* deserialize(Deserializer& des);
   ...
   public:
      ...
      friend Deserializer& operator%(Deserializer& des, StripePattern*& pattern)
      {
         ...
         pattern = StripePattern::deserialize(des);
      ...
      {
         pattern.reset(StripePattern::deserialize(des) );
         return des;
      }
};
