...
REGISTER_TEST_SUITE(TestSerialization);
...
void TestSerialization::testIsDirection()
{
   {
      Serializer ser;
      CPPUNIT_ASSERT(ser.isWriting());
      CPPUNIT_ASSERT(!ser.isReading());
   }

   {
      Deserializer des(0, 0);
      CPPUNIT_ASSERT(!des.isWriting());
      CPPUNIT_ASSERT(des.isReading());
   }
}

void TestSerialization::testPrimitiveTypes()
{
   {
...
void TestSerialization::testPair()
{
   const char expected[] = {
      1, 0, 0, 0,
      1 };
...
void TestSerialization::testPadFieldTo()
{
   const char expected[] = {
      1, 0, 0,
      1, 1,
...

   {
      ...
      ::testPadFieldTo(ser, a, b, c);
      CPPUNIT_ASSERT(ser.good());
      CPPUNIT_ASSERT(memoryEquals(outBuf, expected, sizeof(expected)));
   }

   {
      ...
      CPPUNIT_ASSERT(a == 0x01);
      CPPUNIT_ASSERT(b == 0x0101);
      CPPUNIT_ASSERT(c == 0x01010101);
   }
...
void TestSerialization::testRawString()
{
   {
      const char expected[] = {
         2, 0, 0, 0,
...

   {
...

      {
         ...
         CPPUNIT_ASSERT(ser.size() == sizeof(expectedAligned));
      }

      {
         ...
         ser % serdes::rawString(input, 2, 4);
         CPPUNIT_ASSERT(ser.good());
         CPPUNIT_ASSERT(memoryEquals(outBuf, expectedAligned, sizeof(expectedAligned)));
      }

      {
         ...
         CPPUNIT_ASSERT(des.good());
         CPPUNIT_ASSERT(des.size() == sizeof(expectedAligned));
         CPPUNIT_ASSERT(data == input);
         CPPUNIT_ASSERT(length == 2);
      }
...
void TestSerialization::testBackedPtr()
{
...

   {
      ...
      CPPUNIT_ASSERT(des.good());
      CPPUNIT_ASSERT(des.size() == sizeof(expected));
      CPPUNIT_ASSERT(ptr->value == 1);
      CPPUNIT_ASSERT(ptr == backing.get());
   }
...
void TestSerialization::testRawBlock()
{
   const char expected[] = { 1, 0, 0, 0 };
...
void TestSerialization::testBlock()
{
   ...
   // blocks zeroes memory
   {
      ...
      CPPUNIT_ASSERT(ser.size() == sizeof(buffer));
      CPPUNIT_ASSERT(ser.good());

      for (unsigned i = 0; i < sizeof(buffer); i++)
         CPPUNIT_ASSERT(buffer[i] == 0);
   }
}
...
void TestSerialization::testNonPrimitive()
{
...
void TestSerialization::testCollections()
{
   {
...
void TestSerialization::testAtomic()
{
   const char expected[] = {1, 0, 0, 0};
...
void TestSerialization::testBase()
{
   DerivedType value;
   value.a = 1;
   value.b = 2;
