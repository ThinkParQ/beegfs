#include "TestStorageTk.h"

#include <common/toolkit/StorageTk.h>
#include <tests/TestRunnerBase.h>

REGISTER_TEST_SUITE(TestStorageTk);
...
void TestStorageTk::testFindMountedPrefix()
{
...

   {
      ...
      CPPUNIT_ASSERT(!StorageTk::findLongestMountedPrefix("", file).first);
   }

   {
      std::stringstream file;
      CPPUNIT_ASSERT(!StorageTk::findLongestMountedPrefix("relative", file).first);
   }
...

   {
      ...
      CPPUNIT_ASSERT(res.first);
      CPPUNIT_ASSERT((res.second == Mount{"/dev/root", "/", "tmpfs"}));
   }

   {
      ...
      CPPUNIT_ASSERT(res.first);
      CPPUNIT_ASSERT((res.second == Mount{"/dev/sda1", "/test/foo/bar", "xfs"}));
   }

   {
      ...
      CPPUNIT_ASSERT(res.first);
      CPPUNIT_ASSERT((res.second == Mount{"/dev/sda1", "/test/foo", "ext4"}));
   }

   {
      ...
      CPPUNIT_ASSERT(res.first);
      CPPUNIT_ASSERT((res.second == Mount{"/dev/sda1", "/test/" FULL_SET_RAW, "ext4"}));
   }
