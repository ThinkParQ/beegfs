...
packages="helperd meta mgmtd storage utils          \
	common_package client_devel beeond beeond_thirdparty_gpl utils_devel"
 
client_packages="client_module"
admon_package="admon"
opentk_package="opentk_lib" # deletes the libopentk.so, so needs to be last

export CONCURRENCY=${MAKE_CONCURRENCY:-4}
PACKAGEDIR="/tmp/packages-${DATE}/"
...
print_usage()
{
	...
	echo
	echo "EXAMPLE:"
	echo "  $ `basename $0` -j 4 -p /tmp/my_packages"
	echo
}
