...
class StringTk
{
   ...
   public:
      ...
      //old style (not so good distribution in practice). kept as a reference.
//      static unsigned strChecksum(const char* s)
//      {
...
//      }
      ...
      static unsigned strChecksum(const char* s, int sLen)
      {
         // Note: We use a 32bit checksum here to get the same result (fast) on x86 and x86_64
