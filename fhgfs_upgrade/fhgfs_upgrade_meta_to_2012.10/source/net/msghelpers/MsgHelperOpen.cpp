   ...
   FileInode** outFileInode)
{
   ...
   { // validity check failed => don't open this file (we would risk corrupting it otherwise)
      LogContext(logContext).logErr("This version of FhGFS is not compatible with this "
         ...
         "Refusing to open file parentInfo: " + entryInfo->getParentEntryID() +
   ...
   { // trunc was specified and is needed => do it
      LOG_DEBUG(logContext, 4, std::string("Opening with trunc local") );

      FhgfsOpsErr truncRes = MsgHelperTrunc::truncChunkFile(*outFileInode, entryInfo, 0);
