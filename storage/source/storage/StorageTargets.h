...
{
   public:
      StorageTargets(Config* cfg);
      ~StorageTargets();

      void prepareTargetDirs(Config* cfg);
   ...
   private:
      ...
      void initTargetPathsList(Config* cfg);

      void prepareTargetPaths(Config* cfg);

      void lockTargetPaths();
      void unlockTargetPaths();
   ...
   private:
      ...
      void createBuddyNeedsResyncFile(uint16_t targetID, bool* fileCreated, uint16_t buddyTargetID)
      {
         ...
         if (*fileCreated)
         {
            ...
            LOG(MIRRORING, CRITICAL, "Secondary mirror buddy needs to be resynced.",
