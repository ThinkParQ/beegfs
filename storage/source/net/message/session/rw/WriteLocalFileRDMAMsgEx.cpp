...
bool WriteLocalFileRDMAMsgEx::processIncoming(ResponseContext& ctx)
{
   ...
   if (!rdma->more())
   {
      ctx.sendResponse(WriteLocalFileRDMARespMsg(FhgfsOpsErr_INVAL) );
      return false;
   }
   ...
   if (success)
   {
      ctx.sendResponse(WriteLocalFileRDMARespMsg(writeClientRes) );
...
std::pair<bool, int64_t> WriteLocalFileRDMAMsgEx::write(ResponseContext& ctx)
{
   const char* logContext = "WriteLocalFileRDMAMsgEx incoming";
   ...
   { // session file exists
      ...
      { // server crashed during the write, maybe lost some data send error to client
         LogContext log("WriteLocalFileRDMAMsgEx incoming");
 ...
 */
int64_t WriteLocalFileRDMAMsgEx::incrementalRecvAndWriteStateful(ResponseContext& ctx,
   SessionLocalFile* sessionLocalFile)
{
   const char* logContext = "WriteLocalFileRDMAMsgEx (write incremental)";
   ...
   do
   {
      ...
      recvRes = ctx.getSocket()->read(ctx.getBuffer(), recvLength, 0, rBuf+rOff, rdma->key);
      ...
      rOff += writeRes;
      if (toBeReceived > 0 && rOff == rLen)
      {
         if (!rdma->next(rBuf, rLen, rOff))
         {
            LogContext(logContext).logErr("RDMA buffers expended but not all data received. toBeReceived=" +
               StringTk::uint64ToStr(toBeReceived) + "; "
               "target: " + StringTk::uintToStr(sessionLocalFile->getTargetID() ) + "; "
               "file: " + sessionLocalFile->getFileID() + "; ");
            return getCount() - toBeReceived;
         }
      }
   } while(toBeReceived);
 ...
 */
ssize_t WriteLocalFileRDMAMsgEx::doWrite(int fd, char* buf, size_t count, off_t offset, int& outErrno)
{
 ...
 */
void WriteLocalFileRDMAMsgEx::incrementalRecvPadding(ResponseContext& ctx,
   int64_t padLen, SessionLocalFile* sessionLocalFile)
{
   ...
   while(toBeReceived)
   {
      ...
      recvLength = BEEGFS_MIN(ctx.getBufferLength(), toBeReceived);
      recvLength = BEEGFS_MIN(recvLength, (ssize_t)(rLen - rOff));
      ssize_t recvRes = ctx.getSocket()->read(ctx.getBuffer(), recvLength, 0, rBuf+rOff, rdma->key);
...
FhgfsOpsErr WriteLocalFileRDMAMsgEx::openFile(const StorageTarget& target,
      SessionLocalFile* sessionLocalFile)
{
   const char* logContext = "WriteLocalFileRDMAMsgEx (write incremental)";
 ...
 */
FhgfsOpsErr WriteLocalFileRDMAMsgEx::prepareMirroring(char* buf, size_t bufLen,
   SessionLocalFile* sessionLocalFile, StorageTarget& target)
{
   const char* logContext = "RDMAMsgEx (prepare mirroring)";
 ...
 */
FhgfsOpsErr WriteLocalFileRDMAMsgEx::sendToMirror(const char* buf, size_t bufLen,
   int64_t offset, int64_t toBeMirrored, SessionLocalFile* sessionLocalFile)
{
   const char* logContext = "RDMAMsgEx (send to mirror)";
 ...
 */
FhgfsOpsErr WriteLocalFileRDMAMsgEx::finishMirroring(SessionLocalFile* sessionLocalFile,
      StorageTarget& target)
{
   const char* logContext = "RDMAMsgEx (finish mirroring)";
...
bool WriteLocalFileRDMAMsgEx::doSessionCheck()
