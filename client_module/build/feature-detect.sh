...
wait_jobs()
{
	...
	do
		#echo >&2 "njobs = $njobs"
		job_pids=${job_pids# }  # trim leading space
		pid=${job_pids%% *}  # first word
		job_pids=${job_pids#* }  # remaining words
		#echo >&2 "wait for $pid"
		wait "$pid"
		status=$?
		if [ "$status" != 0 ]
...
run_job check_type       'struct file_lock_core'      KERNEL_HAS_FILE_LOCK_CORE               linux/filelock.h

run_job check_type       'struct proc_ops'            KERNEL_HAS_PROC_OPS                     linux/proc_fs.h

run_job check_type       sockptr_t                    KERNEL_HAS_SOCKPTR_T                    linux/sockptr.h
...
run_job check_type       time64_t                       KERNEL_HAS_TIME64                       linux/ktime.h
run_job check_function   ktime_get_ts64             'void (struct timespec64 *)'  KERNEL_HAS_KTIME_GET_TS64               linux/ktime.h
run_job check_function   ktime_get_real_ts64        'void (struct timespec64 *)'  KERNEL_HAS_KTIME_GET_REAL_TS64          linux/ktime.h
run_job check_function   ktime_get_coarse_real_ts64 'void (struct timespec64 *)'  KERNEL_HAS_KTIME_GET_COARSE_REAL_TS64   linux/ktime.h
