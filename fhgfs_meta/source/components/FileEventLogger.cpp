...
void FileEventLogger::log(FileEvent event, bool missedAnyEvent)
{
   ...
   FileEventLogItem logItem = {droppedEventsSeq, missedEventsSeq, std::move(event)};
   // enough space for two uint64, one uint32 and a 4k string
   char itemBuf[8+8+4+(4+4096)];
