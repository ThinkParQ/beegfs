   ...
   const std::string& name)
{
   ...
   if (!dentry)
   {
      LOG(ERR, "Could not open dentry.", basePath, contDirRelPath, name);
      return FhgfsOpsErr_INTERNAL;
   }
...

   {
      ...
      if (!ser.good())
      {
         LOG(ERR, "Could not serialize dentry for secondary.");
         return FhgfsOpsErr_INTERNAL;
      }
   }
   ...
   const bool isDirectory)
{
   ...
   if (Program::getApp()->getConfig()->getStoreClientXAttrs())
   {
      ...
      {
         LOG(ERR, "Could not list resync candidate xattrs.", fullPath, as("error", xattrs.first));
         xattrs.second.clear();
         return FhgfsOpsErr_INTERNAL;
      }
      ...
      {
         LOG(ERR, "Error while sending xattrs to secondary.", fullPath, xattrRes);
         return FhgfsOpsErr_INTERNAL;
      }
   }
