   ...
   ResponseContext& ctx, bool isSecondary)
{
   ...
   { // check of stripe pattern details validity failed
      LOG(ERR, "Received an invalid pattern chunksize", as("chunkSize", pattern->getChunkSize()));
...
bool SetDirPatternMsgEx::forwardToSecondary(ResponseContext& ctx)
{
   return sendToSecondary(ctx, *this, NETMSGTYPE_SetDirPatternResp) == FhgfsOpsErr_SUCCESS;
}
