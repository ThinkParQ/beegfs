   ...
   bool isSecondary)
{
   ...
   {
      LOG(ERR, "Received a RemoveXAttrMsg, but client-side extended attributes are disabled in config.");
...
bool RemoveXAttrMsgEx::forwardToSecondary(ResponseContext& ctx)
{
   return sendToSecondary(ctx, *this, NETMSGTYPE_RemoveXAttrResp) == FhgfsOpsErr_SUCCESS;
}
