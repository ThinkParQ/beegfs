   ...
   bool isSecondary)
{
   ...
   if (entryInfo->getParentEntryID().empty() || DirEntryType_ISDIR(entryInfo->getEntryType()))
   {
      ...
      if (setAttrRes != FhgfsOpsErr_SUCCESS || !shouldFixTimestamps())
...
finish:
   ...
   // TODO: set needs resync if secondary and path did not exist

   return boost::make_unique<ResponseState>(setAttrRes);
}

/*
 * @return true if normal execution can go ahead; false if retry response was sent to client
 */
bool SetAttrMsgEx::forwardToSecondary(ResponseContext& ctx)
{
   return sendToSecondary(ctx, *this, NETMSGTYPE_SetAttrResp) == FhgfsOpsErr_SUCCESS;
}
