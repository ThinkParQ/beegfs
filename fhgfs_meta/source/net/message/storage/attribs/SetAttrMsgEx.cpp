   ...
   bool isSecondary)
{
   ...
   if (entryInfo->getParentEntryID().empty() || DirEntryType_ISDIR(entryInfo->getEntryType()))
   {
      ...
      if (!isSecondary && getFileEvent() && app->getFileEventLogger())
      {
...
finish:
   ...
   if (!isSecondary && setAttrRes == FhgfsOpsErr_SUCCESS && getFileEvent() &&
         app->getFileEventLogger())
   {
      auto& ev = *getFileEvent();
      app->getFileEventLogger()->log(ev, inode->getNumHardlinks() > 1);
   }
