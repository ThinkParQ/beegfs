...
FhgfsOpsErr ResyncRawInodesMsgEx::removeUntouchedInodes()
{
   ...
   if (!dir)
   {
      LOG(ERR, "Could not open meta directory.", dirPath, sysErr());
      return FhgfsOpsErr_INTERNAL;
   }
   ...
   if (dirFD < 0)
   {
      LOG(ERR, "Could not get directory fd.", sysErr());
      return FhgfsOpsErr_INTERNAL;
   }
   ...
   while (true)
   {
      ...
      if (errno != EISDIR)
      {
         LOG(ERR, "Could not remove file", basePath, found->d_name, sysErr());
         return FhgfsOpsErr_INTERNAL;
      }
      ...
      if (!rmRes)
      {
         LOG(ERR, "Could not remove file", found->d_name, sysErr());
         return FhgfsOpsErr_INTERNAL;
      }
   }
