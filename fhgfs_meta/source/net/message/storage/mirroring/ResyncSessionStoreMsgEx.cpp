...
bool ResyncSessionStoreMsgEx::processIncoming(ResponseContext& ctx)
{
   ...
   {
      LOG(ERR, "Failed to allocate receive buffer for session store resync - out of memory.");
      return false;
   }
   else if (receiveRes == FhgfsOpsErr_COMMUNICATION)
   {
      LOG(ERR, "Failed to receive session store buffer during resync.");
      return false;
   }
   ...
   if (!clearRes)
   {
      LOG(ERR, "Failed to clear session store.");
   ...
   if (!deserRes)
   {
      LOG(ERR, "Failed to deserialize session store data from primary.");
      ctx.sendResponse(ResyncSessionStoreRespMsg(FhgfsOpsErr_INTERNAL));
      return true;
   }
