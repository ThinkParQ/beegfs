...
bool LookupIntentMsgEx::processIncoming(ResponseContext& ctx)
{
   ...
   LOG_DBG(DEBUG, "Received a LookupIntentMsg", as("peer", ctx.peerName()), parentEntryID,
        entryName, as("isBuddyMirrored", getParentInfo()->getIsBuddyMirrored()));

   // sanity checks
   if (unlikely(parentEntryID.empty() || entryName.empty()))
   {
      LOG(WARNING, "Sanity check failed", parentEntryID, entryName);
   ...
   StripePattern** outPattern, PathInfo* outPathInfo, bool isSecondary)
{
   ...
   *outPattern = sessionFile->getInode()->getStripePattern();
   ...
   else
   {
      ...
      if (!addRes)
         LOG(NOTICE, "Couldn't add sessionFile on secondary", as("sessionID", this->getSessionID()),
               sessionFileID);
   }
...
bool LookupIntentMsgEx::forwardToSecondary(ResponseContext& ctx)
{
   addBuddyInfo(entryID, inodeData.getStripePattern());
   return sendToSecondary(ctx, *this, NETMSGTYPE_LookupIntentResp) == FhgfsOpsErr_SUCCESS;
}
