...
bool FLockEntryMsgEx::processIncoming(ResponseContext& ctx)
{
   ...
   LOG_DEBUG(logContext, Log_DEBUG, "Received a FLockEntryMsg from: " + ctx.peerName() );
#endif // BEEGFS_DEBUG

   clientResult = FhgfsOpsErr_INTERNAL;

   Program::getApp()->getNodeOpStats()->updateNodeOp(ctx.getSocket()->getPeerIP(),
         MetaOpCounter_FLOCKENTRY, getMsgHeaderUserID());
...
cleanup_session:
   ...
   LOG_DBG(SPAM, "", getClientNumID(), clientResult);

   return boost::make_unique<ResponseState>(clientResult);
}

bool FLockEntryMsgEx::forwardToSecondary(ResponseContext& ctx)
{
   return sendToSecondary(ctx, *this, NETMSGTYPE_FLockEntryResp, clientResult)
      == FhgfsOpsErr_SUCCESS;
}
