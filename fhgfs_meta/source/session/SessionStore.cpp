...
void SessionStore::deserializeLockStates(Deserializer& des, MetaStore& metaStore)
{
   ...
   if (!des.good())
   {
      LOG(ERR, "Could not restore file inode lock states.");
      return;
   }
}
...
bool SessionStore::deserializeFromBuf(const char* buf, size_t bufLen, MetaStore& metaStore)
{
   ...
   if (!des.good())
   {
      LOG(ERR, "Unable to load lock states.");
      return false;
   }
...
bool SessionStore::loadFromFile(const std::string& filePath, MetaStore& metaStore)
{
   ...
   {
      LOG(WARNING, "Session file exists, but is empty.", filePath);
      return false;
   }
   ...
   if (!buf)
   {
      LOG(ERR, "Could not allocate memory to read session file", filePath);
      goto err_stat;
   }
...
std::pair<std::unique_ptr<char[]>, size_t> SessionStore::serializeToBuf()
{
...

   {
      ...
      if (!buf)
      {
         LOG(ERR, "Could not allocate memory to serialize sessions.");
...

   {
      ...
      if (!ser.good())
      {
         LOG(ERR, "Unable to serialize sessions.");
         return { std::unique_ptr<char[]>(), 0 };
      }
   }
...
bool SessionStore::saveToFile(const std::string& filePath)
{
   ...
   // file created => store data
   {
      ...
      if (!buf.first)
      {
         LOG(ERR, "Unable to save session file", filePath);
         goto err_closefile;
      }
...
bool SessionStore::clear()
{
   ...
   for (SessionMapIter sessionIt = sessions.begin(); sessionIt != sessions.end(); /* inc in body */)
   {
      ...
      if (session)
      {
         ...
         {
            LOG(ERR, "Session file still references in unreferenced session.");
            continue;
         }
      ...
      {
         LOG(ERR, "Session still referenced", as("ID", sessionIt->first));
      }
   }
