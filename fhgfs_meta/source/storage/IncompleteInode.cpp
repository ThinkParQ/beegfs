...
{
   if (fd >= 0 && close(fd) < 0)
      LOG(ERR, "Failed to close file", fd, sysErr());
}
...
FhgfsOpsErr IncompleteInode::clearUnsetXAttrs()
{
   ...
   if (listRes < 0)
   {
      LOG(ERR, "could not list xattrs", fileName(), sysErr());
   ...
   for (auto it = xattrsOnInode.begin(); it != xattrsOnInode.end(); it++)
   {
      ...
      if (removeRes != 0)
      {
         LOG(ERR, "could not remove superfluous xattr", fileName(), as("name", *it), sysErr());
         return FhgfsOpsErr_INTERNAL;
      }
   }
...
std::string IncompleteInode::fileName() const
{
   ...
   if (readRes < 0)
   {
      LOG(ERR, "Failed to resolve file for fd", fd, sysErr());
