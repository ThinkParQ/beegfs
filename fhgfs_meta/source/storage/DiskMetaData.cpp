...
#define DIRECTORY_STORAGE_FORMAT_16BIT  1
#define DIRECTORY_STORAGE_FORMAT_32BIT  2
...
void DiskMetaData::serializeInDentryFormat(Serializer& ser, DiskMetaDataType metaDataType)
{
   ...
   switch (dentryFormatVersion)
   {
      ...
      case DIRENTRY_STORAGE_FORMAT_VER5:
         serializeDentryV5(ser); // inlined inodes + chunk-path-V3
         break;
   }
}
...
void DiskMetaData::deserializeDentry(Deserializer& des)
{
   ...
   { // dentry feature flags
      ...
      {
         des.setBad();
         LOG(ERR, "Incompatible DirEntry feature flags found.", hex(dentryFeatureFlags),
...
void DiskMetaData::deserializeDentryV4(Deserializer& des)
{
...

   {
      ...
      {
         des.setBad();
         LOG(ERR, "Incompatible FileInode feature flags found.", hex(inodeFeatureFlags),
 ...
 */
void DiskMetaData::serializeDentryV5(Serializer& ser)
{
   StatData* statData           = this->inodeData->getInodeStatData();
   StripePattern* stripePattern = this->inodeData->getPattern();
...
void DiskMetaData::deserializeDirInode(Deserializer& des, DirInode& outInode)
{
   ...
   switch (formatVersion)
   {
      case DIRECTORY_STORAGE_FORMAT_16BIT:
      {
         uint16_t ownerNode;
         uint16_t parentNode;
      ...
      case DIRECTORY_STORAGE_FORMAT_32BIT:
         des
