...
QuotaManager::QuotaManager()
 : PThread("QuotaMgr"),
   log("QuotaManager"),
   usedQuotaUser(new QuotaDataMapForTarget() ),
   usedQuotaGroup(new QuotaDataMapForTarget() ),
   exceededQuotaStore(new ExceededQuotaStore() )
{
 ...
 */
void QuotaManager::updateQuotaLimits(const QuotaDataList& list)
{
   if(list.begin()->getType() == QuotaDataType_USER)
      this->quotaUserLimits->addOrUpdateLimits(list);
   else
      this->quotaGroupLimits->addOrUpdateLimits(list);
...
void QuotaManager::calculateExceededQuota(UIntList& uidList, UIntList& gidList)
{
   ...
   QuotaDataMap userLimits = this->quotaUserLimits->getAllQuotaLimits();
   ...
   QuotaDataMap groupLimits = this->quotaGroupLimits->getAllQuotaLimits();
...
bool QuotaManager::pushExceededQuotaIDs()
{
   ...
   return std::find(nodeResults.begin(), nodeResults.end(), false) == nodeResults.end();
}
...
bool QuotaManager::getQuotaLimitsForID(QuotaData& inOutQuotaData)
{
   if(inOutQuotaData.getType() == QuotaDataType_USER)
      return this->quotaUserLimits->getQuotaLimit(inOutQuotaData);
   else
   if(inOutQuotaData.getType() == QuotaDataType_GROUP)
      return this->quotaGroupLimits->getQuotaLimit(inOutQuotaData);
   ...
   QuotaDataType type, QuotaDataList* outQuotaDataList)
{
   if (type == QuotaDataType_USER)
      return this->quotaUserLimits->getQuotaLimitForRange(rangeStart, rangeEnd, outQuotaDataList);
   else
   if (type == QuotaDataType_GROUP)
      return this->quotaGroupLimits->getQuotaLimitForRange(rangeStart, rangeEnd, outQuotaDataList);
