...
bool GenericDebugMsgEx::processIncoming(ResponseContext& ctx)
{
   ...
   LOG_DEBUG_CONTEXT(log, Log_DEBUG, "Received a GenericDebugMsg from: " + ctx.peerName() );
...
std::string GenericDebugMsgEx::processOpListPools(std::istringstream& commandStream)
{
   ...
   NodeCapacityPools* metaBuddyPools = app->getMetaBuddyCapacityPools();
   ...
   {
      std::string metaBuddyPoolsState;

      metaBuddyPools->getStateAsStr(metaBuddyPoolsState);

      responseStream << "META BUDDY POOLS STATE: " << std::endl <<
         metaBuddyPoolsState << std::endl;
   }
   ...
   {
      std::string storageBuddyPoolsState;

      storageBuddyPools->getStateAsStr(storageBuddyPoolsState);

      responseStream << "STORAGE BUDDY POOLS STATE: " << std::endl <<
         storageBuddyPoolsState << std::endl;
   }
