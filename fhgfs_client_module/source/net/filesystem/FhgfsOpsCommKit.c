...
static bool __commkit_prepare_io(CommKitContext* context, struct CommKitTargetInfo* info,
   int events)
{
   if(Thread_isSignalPending() || !Node_getIsActive(info->node) )
   {
   ...
   struct CommKitTargetInfo* info, unsigned requestMsgType)
{
   ...
   switch(GenericResponseMsg_getControlCode(&msg) )
   {
      ...
      case GenericRespMsgCode_INDIRECTCOMMERR_NOTAGAIN:
         if(!info->logged.indirectCommErrorNoRetry)
         {
            info->logged.indirectCommErrorNoRetry = true;
