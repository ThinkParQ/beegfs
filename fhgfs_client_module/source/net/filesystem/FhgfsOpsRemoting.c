...
{
   return FhgfsOpsRemoting_closefileEx(entryInfo, ioInfo, true);
}
   ...
   bool checkSession, bool doSyncOnClose)
{
   ...
   SignalTk_blockSignals(true, &oldSignalSet);
...
state_failed:
   ...
   SignalTk_restoreSignals(&oldSignalSet);
...
FhgfsOpsErr FhgfsOpsRemoting_bumpFileVersion(App* app, const EntryInfo* entryInfo)
{
   ...
   // prepare request
   BumpFileVersionMsg_init(&requestMsg, entryInfo);
