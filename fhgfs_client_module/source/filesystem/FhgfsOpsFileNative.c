...
static int beegfs_flush(struct file* filp, fl_owner_t id)
{
   ...
   bumpRes = FhgfsOpsRemoting_bumpFileVersion(
         FhgfsOps_getApp(filp->f_dentry->d_sb),
         &inode->entryInfo);
...
#else
         result = remove_suid(filp->f_dentry);
#endif
         if(result)
            break;
...
static void __beegfs_readpages_context_free(struct kref* ref)
{
   ...
   FhgfsInode_releaseHandle(BEEGFS_INODE(context->inode), context->handleType);
   kfree(context);
}
...
static BEEGFS_WORK_FUNCTION(beegfs_readpages_work)
{
   ...
   // TODO: handle sparse files better. readpages needn't rely on readpage
