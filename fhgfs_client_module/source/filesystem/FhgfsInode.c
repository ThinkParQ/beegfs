 ...
 */
FhgfsOpsErr FhgfsInode_referenceHandle(FhgfsInode* this, int openFlags, bool allowRWHandle,
   LookupIntentInfoOut* lookupInfo, FileHandleType* outHandleType, uint64_t* outVersion)
{
   ...
   *outHandleType = __FhgfsInode_openFlagsToHandleType(openFlags);
   ...
   { // no matching handle yet => open file to create a new handle
      ...
      {  // file yet opened by lookup or atomic open
         ...
         retVal = FhgfsOpsRemoting_openfile(entryInfo, &ioInfo, outVersion);
